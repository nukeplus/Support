<?php

/**
 * @Project PaCorp 1x
 * @Author PaCorp Co.,Ltd (contact@pacorp.vn)
 * @Copyright (C) 2014 PaCorp Co.,Ltd. All rights reserved
 * @Language Tiếng Việt
 * @License CC BY-SA (http://creativecommons.org/licenses/by-sa/4.0/)
 * @Createdate Mar 04, 2010, 03:22:00 PM
 */

if( ! defined( 'NV_ADMIN' ) or ! defined( 'NV_MAINFILE' ) ) die( 'Stop!!!' );

$lang_translator['author'] = 'PaCorp Co.,Ltd (contact@pacorp.vn)';
$lang_translator['createdate'] = '04/03/2010, 15:22';
$lang_translator['copyright'] = '@Copyright (C) 2012 PaCorp Co.,Ltd. All rights reserved';
$lang_translator['info'] = '';
$lang_translator['langtype'] = 'lang_module';

$lang_module['list'] = 'Danh sách các bài';
$lang_module['add'] = 'Thêm nhân viên';
$lang_module['title'] = 'Tên nhân viên';
$lang_module['order'] = 'Thứ tự';
$lang_module['status'] = 'Trạng thái';
$lang_module['active'] = 'Hoạt động';
$lang_module['inactive'] = 'Ngưng hoạt động';
$lang_module['delete'] = 'Xóa nhân viên';
$lang_module['empty_title'] = 'nhân viên chưa có tiêu đề';
$lang_module['empty_bodytext'] = 'nhân viên chưa có nội dung';
$lang_module['bodytext'] = 'Nội dung';
$lang_module['edit'] = 'Sửa nhân viên';
$lang_module['save'] = 'Lưu nhân viên';
$lang_module['errorsave'] = 'Lỗi hệ thống không cập nhật được nội dung, bạn hãy kiểm tra lại tiêu đề nhân viên có thể bị trùng';
$lang_module['saveok'] = 'Cập nhật nhân viên thành công';
$lang_module['alias'] = 'Liên kết tĩnh';
$lang_module['keywords'] = 'Từ khóa';
$lang_module['feature'] = 'Chức năng';
$lang_module['description'] = 'Miêu tả';
$lang_module['image'] = 'Hình nhân viên';
$lang_module['imagealt'] = 'Chú thích cho hình';
$lang_module['socialbutton'] = 'Các công cụ Like';
$lang_module['socialbuttonnote'] = 'Hiển thị các công cụ Like facebook, G+, Twitter';
$lang_module['activecomm'] = 'Cho phép thảo luận ';

$lang_module['layout_func'] = 'Sử dụng layout';
$lang_module['layout_default'] = 'Mặc định';
$lang_module['googleplus'] = 'xác thực Google+ (quyền tác giả)';
$lang_module['googleplus_1'] = 'Không sử dụng';
$lang_module['googleplus_0'] = 'Theo cấu hình quyền tác giả của module';

$lang_module['config'] = 'Cấu hình';
$lang_module['config_view_type'] = 'Phương án hiển thị nhân viên';
$lang_module['config_view_type_0'] = 'Một nhân viên chính';
$lang_module['config_view_type_1'] = 'Danh sách các nhân viên';
$lang_module['config_view_type_page'] = 'Số nhân viên hiển thị trên một trang';
$lang_module['config_view_related_articles'] = 'Số nhân viên liên quan';
$lang_module['config_facebookapi'] = 'Facebook App ID';
$lang_module['config_facebookapi_note'] = ' (Có dạng 1419186468293063, <a href="http://wiki.nukeviet.vn/nukeviet:admin:news:facebookapi" target="_blank">xem chi tiết</a>)';
$lang_module['config_save'] = 'Lưu cấu hình';
